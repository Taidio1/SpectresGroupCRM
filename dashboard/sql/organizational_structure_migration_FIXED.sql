-- =====================================================================================
-- MIGRACJA BAZY DANYCH DO STRUKTURY ORGANIZACYJNEJ - NAPRAWIONA WERSJA
-- Spectres Group CRM - Hierarchia: Szef → Kraje → Project Manager → Junior Manager → Pracownicy
-- Wszystkie błędy składni zostały naprawione!
-- =====================================================================================

-- ⚠️ BACKUP: Zrób backup bazy przed uruchomieniem!
-- ⚠️ TEST: Najpierw przetestuj na kopii testowej!
...
-- ---------------------------------------------------------------------------------
-- KROK 5: FUNKCJE POMOCNICZE DLA HIERARCHII (NAPRAWIONE!)
-- ---------------------------------------------------------------------------------

-- NAPRAWIONA: Funkcja do pobierania wszystkich podwładnych użytkownika (rekurencyjnie)
...
-- NAPRAWIONA: Funkcja do pobierania scieżki hierarchii dla użytkownika
...
WITH RECURSIVE hierarchy AS (
    ...
    SELECT u.id, u.full_name, u.role, u.role_hierarchy_level, u.manager_id, h.path_level + 1
    FROM users u
...
-- ---------------------------------------------------------------------------------
-- KROK 8: MATERIALIZED VIEWS DLA WYDAJNOŚCI (NAPRAWIONY!)
-- ---------------------------------------------------------------------------------

-- NAPRAWIONY: Widok zmaterializowany dla statystyk hierarchicznych
...
-- Indeks dla materializowanego widoku
...
-- NASTĘPNY KROK: Uruchom validate_migration.sql do sprawdzenia rezultatu! 